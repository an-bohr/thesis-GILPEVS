...
class Attention(nn.Module):
    def __init__(self, query_dim, context_dim = None, heads = 8, dim_head = 64, drop_path_rate = 0.0):
        ...
        # NOTE: hard_coding inner_dim size for testing acceleration
        # inner_dim = 256
...
class AutoEncoder(nn.Module):
    ...
    def forward(self, pc, queries):
        ...
        if self.output_dim == 2:
            # decode to 2 channels: (B, Q, 2)
            stats = self.decode(x, queries)         # shape [B, num_queries, 2]
            mean, logvar = stats.unbind(dim=-1)     # shape [B, num_queries]
            
            # clamp logvar to avoid large variances and very unstable training:
            logvar = torch.clamp(logvar, min=-7.0, max=4.0)
            
            return {'logits': mean, 'logvar': logvar}
        else:
...
def create_autoencoder(dim=512, M=512, latent_dim=64, N=2048, determinisitc=False, depth=24, output_dim=1):
    if determinisitc:
        model = AutoEncoder(
            depth=depth,
            dim=dim,
...
######################
# Testing with own small model:
def ae_d8_m64(N=2048):
    return create_autoencoder(dim=8, M=64, N=N, determinisitc=True)
# Reduce number of latents, depth:
def ae_d64_256_depth12(N=2048):
    return create_autoencoder(dim=64, M=256, N=N, determinisitc=True, depth=12)

def ae_d64_256_depth6(N=2048):
    return create_autoencoder(dim=64, M=256, N=N, determinisitc=True, depth=6)
# Testing with small VAE:
def kl_d64_m256_l4(N=2048):
    return create_autoencoder(dim=64, M=256, latent_dim=4, N=N, determinisitc=False, depth=12)
# For NLL:
def ae_d64_256_depth6_outdim2(N=2048):
    return create_autoencoder(dim=64, M=256, N=N, determinisitc=True, depth=6, output_dim=2)        # Changed to output [mean, logvar]